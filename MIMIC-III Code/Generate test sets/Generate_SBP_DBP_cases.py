...
save_dir = 'DBP SBP set - Seed3 - SD 15/'
...
SBP = np.linspace(0, 350, 70).astype(int)

'''
This function  generates normal distribution 
using two boundaries 
'''
def create_DBP_distribution(mu, sigma = 15, count = 53):
    a = max(mu-sigma/2, 0)
    b = mu+sigma/2
...
for i in range(0, 70):
    ...
    values = create_DBP_distribution(mu=SBP[i], sigma=sigma, count=case.shape[0])
    case['Systolic blood pressure'] = values
    print(SBP[i], "SBP Mean", sum(values) / len(values), "SD", statistics.pstdev(values))

    for j in range(0, 50):

        values = create_DBP_distribution(mu=DBP[j], sigma=sigma, count=case.shape[0])
        case['Diastolic blood pressure'] = values
        print(DBP[j], "DBP Mean", sum(values) / len(values), "SD", statistics.pstdev(values))
        ...
        name = 'Real_Seed3_' + 'DBP_' + str(DBP[j]) + '_SBP_' + str(SBP[i]) + '.csv'
