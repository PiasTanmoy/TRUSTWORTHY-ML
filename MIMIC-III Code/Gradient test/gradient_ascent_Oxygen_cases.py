...
for idx in range(data.shape[0]):
    ...
    while l11 == l22 == l33:
        ...
        attribute_val = np.average(gen_img_48_76[ :, oxy_col])
    ...
    if flag_save:
        print("-------------------------------------------------------------------------")
        np.savetxt("Gradients2/Oxygen/Modified_" + names[idx], gen_img_48_76, delimiter=",")
        ...
        import pandas as pd
        dd = pd.DataFrame(d)
        dd.to_csv("Gradients2/Oxygen/Pred_" + names[idx] + flag + ".csv", index = False)
