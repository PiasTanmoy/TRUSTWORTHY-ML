...
for idx in range(data.shape[0]):
    ...
    while l11 == l22 == l33:
        ...
        temp = np.average(gen_img_48_76[ :, temp_col])
        heart = np.average(gen_img_48_76[:, heart_col])
        resp = np.average(gen_img_48_76[:, resp_col])
        DBP = np.average(gen_img_48_76[:, DBP_col])
        SBP = np.average(gen_img_48_76[:, SBP_col])
        oxy = np.average(gen_img_48_76[:, oxy_col])
        glu = np.average(gen_img_48_76[:, glu_col])

        attribute_val_list.append([temp, heart, resp, DBP, SBP, oxy, glu])


        min_attribute_val = min(temp, heart, resp, DBP, SBP, oxy, glu)

        if min_attribute_val <= 0.0:
            flag = "X"
            break

        all.append([temp, heart, resp, DBP, SBP, oxy, glu, p11, p22, p33])

        gen_img = normalizer.transform(gen_img_48_76)
        gen_img = gen_img.reshape(1, 48, 76)

        print(con, temp, heart, resp, DBP, SBP, oxy, glu, p11, p22, p33, l11, l22, l33)

    if flag_save:
        print("-------------------------------------------------------------------------")
        np.savetxt("Gradients2/Temperature/Modified_" + names[idx], gen_img_48_76, delimiter=",")

        d = {
            "attributes": attribute_val_list,
            "C1": p11_list,
            "C2": p22_list,
            "C3": p33_list
        }
        ...
        import pandas as pd
        dd = pd.DataFrame(d)
        dd.to_csv("Gradients2/Temperature/Pred_" + names[idx] + flag + ".csv", index = False)
