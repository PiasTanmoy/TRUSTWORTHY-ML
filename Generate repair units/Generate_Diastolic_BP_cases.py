...
for fileName in numFiles:
    ...
    DBP_val = values = create_DBP_distribution(mu=i, sigma=15, count=case.shape[0])

    case['Diastolic blood pressure'] = values
    case['Glascow coma scale total'] = case['Glascow coma scale total'].astype('Int64')

    name = fileName[:-4] + '_DBP_' + str(i) + '.csv'
    case.to_csv(pathName+'/DBP_C1_low_2k/' + name, index=False)
    print("SAEVD", name)
...
df.to_csv(pathName+'/DBP_C1_low_2k_name_list.csv', index=False)
