...
for fileName in numFiles:
    ...
    i = random.randint(39, 42)
    DBP_val = values = create_respiratory_rate_distribution(mu=i, sigma=0.7, count=case.shape[0])

    case['Temperature'] = values
    case['Glascow coma scale total'] = case['Glascow coma scale total'].astype('Int64')

    name = fileName[:-4] + '_Temp_' + str(i) + '.csv'
    case.to_csv(pathName+'/Temp_C1_high_2k/' + name, index=False)
    print("SAEVD", name)
...
df.to_csv(pathName+'/Temp_C1_high_2k_name_list.csv', index=False)
