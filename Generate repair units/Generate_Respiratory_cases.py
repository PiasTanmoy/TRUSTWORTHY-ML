...
for fileName in numFiles:
    ...
    DBP_val = values = create_respiratory_rate_distribution(mu=i, sigma=5, count=case.shape[0])

    case['Respiratory rate'] = values
    case['Glascow coma scale total'] = case['Glascow coma scale total'].astype('Int64')

    name = fileName[:-4] + '_Resp_' + str(i) + '.csv'
    case.to_csv(pathName+'/Resp_C1_low_2k_1_10/' + name, index=False)
    print("SAEVD", name)
...
df.to_csv(pathName+'/Resp_C1_low_2k_1_10_name_list.csv', index=False)
