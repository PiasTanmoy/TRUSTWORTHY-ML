...
for idx in range(data.shape[0]):
    ...
    while l11 == l22 == l33:
        ...
        DBP_val = np.average(gen_img_48_76[ :, DBP_col])
        ...
        all.append([DBP_val, p11, p22, p33])

        gen_img = normalizer.transform(gen_img_48_76)
        gen_img = gen_img.reshape(1, 48, 76)

        print(con, DBP_val, p11, p22, p33, l11, l22, l33)

    if flag_save:
        print("-------------------------------------------------------------------------")
        np.savetxt("Gradients2/Trial/Modified_" + names[idx], gen_img_48_76, delimiter=",")
        ...
        import pandas as pd
        dd = pd.DataFrame(d)
        dd.to_csv("Gradients2/Trial/Pred_" + names[idx] + flag + ".csv", index = False)
